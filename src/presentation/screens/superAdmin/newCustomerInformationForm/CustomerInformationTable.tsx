...
const CustomerInformationTable: React.FC = () => {
  const { fetchCustomerInformation, deleteCustomerInformation, isLoading } =
  ...
  const handleDeleteClick = ( customer: CustomerInfo ) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    const id = customerToDelete.recordId || customerToDelete.formId;
    if ( !id ) return;
    const success = await deleteCustomerInformation( id );
    if ( success ) {
      setCustomers( ( prev ) =>
        prev.filter( ( c ) => ( c.recordId || c.formId ) !== id )
      );
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6 mb-8">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                { customers.map( ( customer ) => (
                  <tr
                    key={ customer.recordId || customer.formId || "no-id" }
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                      ...
                      { customer.pricing != null && (
                        <span className="ml-1 text-xs text-indigo-600 dark:text-indigo-400">
                          · { customer.pricing.toLocaleString( "es-MX", { style: "currency", currency: "MXN" } ) }
                        </span>
                      ) }
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                ...
                <button
                  ...
                  className="mt-3 sm:mt-0 w-full inline-flex justify-center items-center gap-1.5 rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:w-auto sm:text-sm"
                >
      ...
      <DeleteConfirmationModal
        ...
        message={ `¿Estás seguro de que deseas eliminar el registro de ${ customerToDelete?.name ?? "" } ${ customerToDelete?.lastName ?? "" }? Esta acción eliminará el documento de Firestore y no se puede deshacer.` }
