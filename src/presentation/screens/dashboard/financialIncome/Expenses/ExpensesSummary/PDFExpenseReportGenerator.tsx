...
const PDFExpenseReportGenerator: React.FC<PDFExpenseReportGeneratorProps> = ( {
  ...
  const { expenses, totalSpent, monthlyStats } = useExpenseSummaryStore(
    ( state ) => ( {
  ...
  const generatePDF = async () => {
    ...
    const pageWidthExp = doc.internal.pageSize.getWidth();
    const pageHeightExp = doc.internal.pageSize.getHeight();
    for ( let i = 1; i <= totalPages; i++ ) {
      doc.setPage( i );
      doc.setFontSize( 9 );
      doc.setTextColor( 120 );
      doc.text( `EstateAdmin - ${ generatedAt }`, pageWidthExp / 2, pageHeightExp - 8, { align: "center" } );
      doc.text( `Página ${ i } de ${ totalPages }`, pageWidthExp - 14, pageHeightExp - 8, { align: "right" } );
    }
