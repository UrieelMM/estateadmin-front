...
const PDFBalanceGeneralReport: React.FC<PDFBalanceGeneralReportProps> = ( {
  ...
  const generatePDF = async () => {
    ...
    const stats = [
      `• Mes con mayores ingresos: ${ monthNames[ analisisAvanzado.mesMaxIngreso?.month || "01" ]
      } (${ formatValue(
        analisisAvanzado.mesMaxIngreso?.paid +
        analisisAvanzado.mesMaxIngreso?.saldo
      ) })`,
      `• Mes con mayores egresos: ${ monthNames[ analisisAvanzado.mesMaxEgreso?.month || "01" ]
      } (${ formatValue( analisisAvanzado.mesMaxEgreso?.spent ) })`,
      `• Mes con mejor balance: ${ monthNames[ analisisAvanzado.mejorMes?.month || "01" ]
    ...
    const pageWidthBal = doc.internal.pageSize.getWidth();
    const pageHeightBal = doc.internal.pageSize.getHeight();
    for ( let i = 1; i <= totalPages; i++ ) {
      doc.setPage( i );
      doc.setFontSize( 9 );
      doc.setTextColor( 120 );
      doc.text( `EstateAdmin - ${ generatedAt }`, pageWidthBal / 2, pageHeightBal - 8, { align: "center" } );
      doc.text( `Página ${ i } de ${ totalPages }`, pageWidthBal - 14, pageHeightBal - 8, { align: "right" } );
    }
