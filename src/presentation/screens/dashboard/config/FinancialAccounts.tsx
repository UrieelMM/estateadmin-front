...
import { BuildingOffice2Icon, BanknotesIcon, CurrencyDollarIcon, ChatBubbleBottomCenterTextIcon } from "@heroicons/react/24/solid";
...
const FinancialAccounts = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormData({ name: "", type: "", description: "", initialBalance: 0, active: true });
      setIsEditing(false);
      setEditingId(null);
      fetchAccounts();
    } catch (error: any) {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-semibold text-black mb-4 dark:text-gray-100">Cuentas Financieras</h2>
      {error && <p className="text-red-600 mb-4">{error}</p>}
      <form onSubmit={handleSubmit} className="mb-6 bg-white shadow-md rounded-lg p-6 dark:bg-gray-800">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Nombre de la cuenta */}
          <div>
            <label className="block text-gray-700 font-medium mb-2 dark:text-gray-100">Nombre de la cuenta</label>
          ...
          {/* Tipo de cuenta */}
          <div>
            <label className="block text-gray-700 font-medium mb-2 dark:text-gray-100">Tipo de cuenta</label>
            <div className="relative">
          ...
          {/* Saldo Inicial */}
          <div>
            <label className="block text-gray-700 font-medium mb-2 dark:text-gray-100">Saldo Inicial</label>
            <div className="relative">
          ...
          {/* Descripción */}
          <div>
            <label className="block text-gray-700 font-medium mb-2 dark:text-gray-100">Descripción (opcional)</label>
            <div className="relative">
        ...
        <div className="mt-6 flex justify-end">
          <button
            type="submit"
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg flex items-center transition-colors duration-200"
          >
      ...
      {loading ? (
        ...
        <div className=" bg-white shadow-md rounded-lg p-6 dark:bg-gray-800">
          <h3 className="text-xl font-semibold mb-4 dark:text-gray-100">Lista de Cuentas</h3>
          {accounts.length === 0 ? (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b">
                  <th className="py-3 px-4 text-left font-medium text-gray-700 dark:text-gray-100">Nombre</th>
                  <th className="py-3 px-4 text-left font-medium text-gray-700 dark:text-gray-100">Tipo</th>
                  <th className="py-3 px-4 text-left font-medium text-gray-700 dark:text-gray-100">Saldo Inicial</th>
                  <th className="py-3 px-4 text-left font-medium text-gray-700 dark:text-gray-100">Activo</th>
                  <th className="py-3 px-4 text-left font-medium text-gray-700 dark:text-gray-100">Acciones</th>
                </tr>
              </thead>
              <tbody>
                {accounts.map((account) => (
                  <tr key={account.id} className="border-b hover:bg-gray-50 transition-colors dark:hover:bg-gray-700 cursor-pointer">
                    ...
                    <td className="py-3 px-4">{account.active ? "Sí" : "No"}</td>
