...
const NewQuoteModal: React.FC<NewQuoteModalProps> = ({
  ...
  const { addProjectQuote, loading } = useProjectStore();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const data: ProjectQuoteCreateInput = {
        projectId,
      ...
      await addProjectQuote(data);
      toast.success("Cotización agregada correctamente");

      // Limpiar el formulario y cerrar el modal
      resetForm();
    } catch (error: any) {
      toast.error(error.message || "Error al agregar la cotización");
    }
  };
        ...
        <div className="flex min-h-full items-center justify-center p-4">
          <Dialog.Panel className="mx-auto max-w-3xl w-full rounded-lg bg-white p-6 dark:bg-gray-800 dark:text-gray-100 shadow-xl">
            <Dialog.Title className="text-lg font-medium border-b pb-2 mb-4 dark:border-gray-700">
              Nueva Cotización para {projectName}
            </Dialog.Title>
