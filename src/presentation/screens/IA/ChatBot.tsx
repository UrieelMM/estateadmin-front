...
// importamos el componente HOC y el store de plan
 ...
 * Solo se muestra para usuarios Pro/Enterprise que no tienen acceso a esta función específica
 */
...
const ChatBot: React.FC = () => {
  // Para depuración, obtenemos el estado del plan
  const { error, clientId, condominiumId, fetchClientPlan } =
    useClientPlanStore();

  // Solo recargar el plan si hay un error específico y solo una vez
  const errorRetryAttempted = useRef(false);

  useEffect(() => {
    // Si hay error genuino, intentar recargar el plan una sola vez
    if (error && clientId && condominiumId && !errorRetryAttempted.current) {
      errorRetryAttempted.current = true;

      // Esperar 2 segundos antes de reintentar para evitar bucles
  ...
  }, [error, clientId, condominiumId, fetchClientPlan]);
...
`;
  ...
  // Protegemos el componente con nuestro HOC
  return (
    <ProtectedProFeature
      featureName="chatbot"
