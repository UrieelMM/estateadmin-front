...
const CardsHomeSummary: React.FC = () => {
  ...
  useEffect(() => {
    const loadDataIfNeeded = async () => {
      ...
      try {
        ...
        if (monthlyStats.length === 0 || shouldFetchData(selectedYear)) {
          setLoading(true);
    ...
    monthlyStats.length,
    setLoading,
    hasCondominiumId, // Añadir dependencia
  ]);
  ...
  // Mostrar skeleton cuando está cargando o no hay condominiumId
  if (loading || !hasCondominiumId) {
    return (
